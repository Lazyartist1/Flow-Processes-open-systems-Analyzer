document.addEventListener("DOMContentLoaded", () => {
  ...
  PROCESS_TYPES.forEach(p => {
    ...
    opt.value = p;
    opt.textContent = PROCESS_DISPLAY_NAMES[p];
  ...
  function renderProcessCards() {
    ...
    PROCESS_TYPES.forEach(p => {
      ...
      card.className = 'card';
      card.textContent = PROCESS_DISPLAY_NAMES[p];
      card.tabIndex = 0;
      card.onclick = () => {
  ...
  form.addEventListener('submit', (ev) => {
    ...
    resultsGrid.innerHTML = '<p style="grid-column: 1/-1; color: var(--text-secondary); text-align: center; padding: 2rem;">Calculating...</p>';
    pvDiv.innerHTML = '';
    tsDiv.innerHTML = '';
    ...
    .then(data => {
      ...
      const resultCards = [
        ...
        { label: 'Flow Work (W_open)', value: result.W_open.toFixed(4), unit: 'kJ' },
        { label: 'Heat Tr. (Q)', value: result.Q.toFixed(4), unit: 'kJ' },
        { label: 'Enthalpy (ΔH)', value: result.deltaH.toFixed(4), unit: 'kJ' },
        { label: 'Entropy (ΔS)', value: result.deltaS.toFixed(6), unit: 'kJ/K' },
      ];
      ...
      resultsGrid.innerHTML = resultCards.map(item => `
        <div class="result-card">
          ...
          <div class="result-card-label" style="font-size: 0.75rem; color: var(--text-secondary); margin-top: auto; padding-top: 0.5rem;">${item.unit}</div>
        </div>
      `).join('');
      ...
      if (pvDataArr.length > 0) {
        ...
        const trace = { x, y, mode: 'lines', name: 'P-V', line: {shape: 'spline', color: '#2563eb', width: 3} };
        const layout = { 
          xaxis: { title: 'Volume V (m³)', gridcolor: '#e6e9ee' }, 
          yaxis: { title: 'Pressure P (kPa)', gridcolor: '#e6e9ee' }, 
        ...
        pvDownload.onclick = async (e) => {
          ...
          pvDownload.download = `pv_flow_plot_${Date.now()}.png`;
        };
      }
      ...
      if (tsDataArr.length > 0) {
        ...
        const trace = { x, y, mode: 'lines', name: 'T-S', line: {shape: 'spline', color: '#ef4444', width: 3} };
        const layout = { 
          xaxis: { title: 'Entropy S (kJ/K)', gridcolor: '#e6e9ee' }, 
          yaxis: { title: 'Temperature T (K)', gridcolor: '#e6e9ee' }, 
          margin: {t:30, l:60, r:30, b:50},
          plot_bgcolor: 'rgba(0,0,0,0)',
          paper_bgcolor: 'rgba(0,0,0,0)'
        };
        ...
        tsDownload.onclick = async (e) => {
          ...
          tsDownload.download = `ts_flow_plot_${Date.now()}.png`;
        };
      }
    })
    .catch(err => {
      resultsGrid.innerHTML = `<div style="grid-column: 1/-1; color: var(--error); padding: 1.5rem; text-align: center; background: rgba(239,68,68,0.06); border-radius: 8px; border: 1px solid var(--error);"><b>Error:</b> ${err.message || err}</div>`;
    })
    .finally(() => {
      submitBtn.disabled = false;
      submitBtn.textContent = prevText;
  ...
  function setControlVisibility(type) {
    ...
    ['V2','T2','n'].forEach(k => {
      ...
      if (cfg[k]) {
        ...
        el.parentElement.style.display = 'none'; // hide entirely for cleaner UI
        el.value = '';
      }
    });
  }
