...
def calculate_process(process_type: ProcessType, substance: str, input_data: ProcessInput, mass: float = 1.0) -> ProcessResult:
    """Compute final state and energetic quantities for an open thermodynamic process (steady-flow).

    Notes:
    - P in kPa, V in m^3, T in K
    - Energies in kJ (P [kPa] * V [m^3] = kJ)
    - W_open represents technical or flow work (-vdP)
    """
    ...
    P2 = V2 = T2 = 0.0
    W_open = Q = deltaH = deltaU = deltaS = 0.0
    ...
    if process_type == "constantVolume":
        ...
        P2 = P1 * (T2 / T1)
        # Open system work: -int(v dP) = -v * (P2 - P1)
        W_open = -V1 * (P2 - P1)
        deltaU = mass * Cv * (T2 - T1) / 1000.0
        deltaH = mass * Cp * (T2 - T1) / 1000.0
        Q = deltaH + W_open
        ...
        P2 = P1 * V1 / V2
        # Open work for isothermal ideal gas: W = P1 V1 ln(V2/V1) = P1 V1 ln(P1/P2)
        if V2 / V1 <= 0:
            raise ValueError("Volume ratio must be positive for log calculation")
        W_open = P1 * V1 * log(V2 / V1)
        deltaU = 0.0
        deltaH = 0.0
        Q = W_open
        ...
        deltaU = mass * Cv * (T2 - T1) / 1000.0
        deltaH = mass * Cp * (T2 - T1) / 1000.0
        # W_open = -deltaH = (gamma/(gamma-1))*(P1V1 - P2V2)
        W_open = (gamma / (gamma - 1.0)) * (P1 * V1 - P2 * V2)
        deltaS = 0.0
